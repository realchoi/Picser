...
enum PurchaseSecretsProvider {
  ...
  private enum InfoKey {
    static let sharedSecret = "PICSER_IAP_SHARED_SECRET"
    static let lifetimeProductIdentifier = "PICSER_IAP_LIFETIME_ID"
    static let subscriptionProductIdentifier = "PICSER_IAP_SUBSCRIPTION_ID"
    static let legacyPrimaryProductIdentifier = "PICSER_IAP_PRODUCT_ID"
  }
  ...
  static func purchaseSharedSecret() -> String? {
    ...
    if let envSecret = ProcessInfo.processInfo.environment["PICSER_IAP_SHARED_SECRET"], !envSecret.isEmpty {
      return envSecret
    }
  ...
  static func purchaseLifetimeIdentifier() -> String {
    if let envProduct = ProcessInfo.processInfo.environment["PICSER_IAP_LIFETIME_ID"], !envProduct.isEmpty {
      return envProduct
    }
    ...
    if let legacyEnv = ProcessInfo.processInfo.environment["PICSER_IAP_PRODUCT_ID"], !legacyEnv.isEmpty {
      return legacyEnv
    }
  ...
  static func purchaseSubscriptionIdentifier() -> String {
    if let envProduct = ProcessInfo.processInfo.environment["PICSER_IAP_SUBSCRIPTION_ID"], !envProduct.isEmpty {
      return envProduct
    }
